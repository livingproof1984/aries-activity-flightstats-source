...
export default class FlightStatsSource extends Activity {
    ...
    async getTaxiData({ airport, embark, date, appId, appKey, utc }) {
        const { year, month, day, hour } = date || this.getCurrentDateObject();

        const path = `flightstatus/rest/v2/json/airport/status/${airport}/${embark}/${year}/${month}/${day}/${hour}`;
        const uri = `${baseURI}${path}`;
        const params = {
            uri: uri,
            qs: { appId, appKey, utc },
    ...
    getUniqueTaxiData(response) {
        return response.flightStatuses.map(status => {
            // Grab some fields off status.
            const statusData = pick(status, [
                'flightId',
                'carrierFsCode',
                'flightNumber',
            ]);

            // Grab flight equipment object.
            const flightEquipment = status.flightEquipment;

            // Grab flight arrival date object.
            const arrivalDate = status.arrivalDate;

            // Grab flight departure date object.
            const departureDate = status.departureDate;

            // Grab flight durations if exists, else defaults.
            const taxiData = status.flightDurations || {
        ...
        //                             { flightId: response.flightStatuses[i].flightId },
        //                             taxiData,
        //                             response.flightStatuses[i].carrierFsCode,
        //                             response.flightStatuses[i].flightNumber,
        //                             response.flightStatuses[i].flightEquipment,
        //                             { timestamp : new Date() });
